import json
from typing import Dict, List, Optional


def build_fused_query(
    model1_output: Optional[Dict],
    model2_output: Optional[Dict],
    doctor_note_output: Optional[Dict] = None,
) -> str:
    query_parts = []

    if model1_output:
        query_parts.append("Image findings:")
        query_parts.append(model1_output.get("patient_summary_text", ""))

        positive_labels = model1_output.get("xray_positive_labels")
        if positive_labels:
            query_parts.append("Positive X-ray labels:")
            query_parts.append(", ".join(str(label) for label in positive_labels))
        else:
            query_parts.append("Positive X-ray labels: none")

        top_predictions = model1_output.get("top_predictions", [])

        for item in top_predictions[:5]:
            query_parts.append(
                f"{item.get('label')}: {item.get('probability')}"
            )

    if model2_output:
        query_parts.append("Document findings:")
        query_parts.append(model2_output.get("patient_summary", ""))
        query_parts.append(model2_output.get("raw_text_preview", ""))

        entities = model2_output.get("entities", [])

        for entity in entities[:20]:
            query_parts.append(str(entity))

    if doctor_note_output:
        query_parts.append("Doctor note findings:")
        query_parts.append(doctor_note_output.get("patient_summary_text", ""))
        query_parts.append(doctor_note_output.get("clean_text", ""))
        query_parts.append(json.dumps(doctor_note_output.get("entities", {}), ensure_ascii=False))

    return "\n".join(query_parts)


def summarize_retrieved_evidence(retrieved_evidence: List[Dict]) -> str:
    if not retrieved_evidence:
        return "No external knowledge-base evidence was retrieved."

    lines = []

    for index, evidence in enumerate(retrieved_evidence, start=1):
        source = evidence.get("source", "unknown source")
        score = evidence.get("score", 0.0)
        text = evidence.get("text", "")

        lines.append(
            f"Evidence {index}: source={source}, score={score:.3f}. "
            f"Relevant note: {text[:300]}..."
        )

    return "\n".join(lines)


def generate_doctor_feedback(
    model1_output: Optional[Dict],
    model2_output: Optional[Dict],
    doctor_note_output: Optional[Dict],
    retrieved_evidence: List[Dict],
    missing_information_notes: Optional[List[str]] = None,
) -> str:
    feedback = []

    feedback.append("This AI-generated output should be reviewed by a qualified clinician.")

    if model1_output:
        feedback.append(
            "Image-based findings are probabilistic and should be verified with clinical context."
        )

    if model2_output:
        feedback.append(
            "OCR-extracted document findings may contain recognition errors, especially with scanned or handwritten reports."
        )

    if doctor_note_output:
        feedback.append(
            "Doctor-provided clinical text was summarized with rule-based extraction and should be checked against the original note."
        )

    if retrieved_evidence:
        feedback.append(
            "Retrieved knowledge-base evidence was used to provide additional context."
        )
    else:
        feedback.append(
            "No knowledge-base evidence was available, so the result is based only on model outputs."
        )

    if missing_information_notes:
        feedback.append(" ".join(missing_information_notes))

    return " ".join(feedback)


def generate_final_summary(
    case_id: str,
    model1_output: Optional[Dict],
    model2_output: Optional[Dict],
    doctor_note_output: Optional[Dict],
    retrieved_evidence: List[Dict],
) -> Dict:
    image_findings = None
    document_findings = None
    doctor_note_findings = None
    available_modalities: List[str] = []
    missing_information_notes: List[str] = []

    summary_parts = []

    summary_parts.append(f"Case ID: {case_id}")

    if model1_output:
        available_modalities.append("image")
        image_findings = model1_output.get("patient_summary_text")
        summary_parts.append(f"Image findings: {image_findings}")
    else:
        missing_information_notes.append("No image input was provided.")
        summary_parts.append("Image findings: No image input was provided.")

    if model2_output:
        available_modalities.append("document")
        document_findings = model2_output.get("patient_summary")
        summary_parts.append(f"Document findings: {document_findings}")
    else:
        missing_information_notes.append("No document input was provided.")
        summary_parts.append("Document findings: No document input was provided.")

    if doctor_note_output:
        available_modalities.append("doctor_note")
        doctor_note_findings = doctor_note_output.get("patient_summary_text")
        summary_parts.append(f"Doctor note findings: {doctor_note_findings}")
    else:
        missing_information_notes.append("No doctor note was provided.")
        summary_parts.append("Doctor note findings: No doctor note was provided.")

    evidence_summary = summarize_retrieved_evidence(retrieved_evidence)
    summary_parts.append(f"Retrieved evidence: {evidence_summary}")

    final_summary = "\n\n".join(summary_parts)
    fused_findings = " ".join(
        part for part in [image_findings, document_findings, doctor_note_findings] if part
    )
    patient_summary = " ".join(
        [
            f"Available modalities: {', '.join(available_modalities) if available_modalities else 'none'}.",
            fused_findings or "No modality-specific findings were available.",
        ]
    )

    doctor_oriented_feedback = generate_doctor_feedback(
        model1_output=model1_output,
        model2_output=model2_output,
        doctor_note_output=doctor_note_output,
        retrieved_evidence=retrieved_evidence,
        missing_information_notes=missing_information_notes,
    )

    return {
        "case_id": case_id,
        "available_modalities": available_modalities,
        "image_findings": image_findings,
        "document_findings": document_findings,
        "doctor_note_findings": doctor_note_findings,
        "text_findings": document_findings,
        "fused_findings": fused_findings,
        "retrieved_evidence": retrieved_evidence,
        "patient_summary": patient_summary,
        "doctor_oriented_feedback": doctor_oriented_feedback,
        "missing_information_notes": missing_information_notes,
        "final_summary": final_summary,
        "doctor_feedback": doctor_oriented_feedback,
    }
